...
df = pd.read_csv('vb_orders.csv',
                   dtype={'user_id': str,
                          'id': str},
                   parse_dates=['created_at'], 
                   infer_datetime_format=True)

#df = df.loc[df['created_at'] >= '2021-01-01 00:00:00' ]

#print(df)
#df.head()
n_orders = df.groupby(['user_id'])['id'].nunique()
...
#Cohort Analysis
df = df[['user_id', 'id', 'created_at']].drop_duplicates()

df['order_month'] = df['created_at'].dt.to_period('M')
df['cohort'] = df.groupby('user_id')['created_at'] \
                 .transform('min') \
                 .dt.to_period('M') 
