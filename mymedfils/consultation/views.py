...
from .models import Consultation, ConsultationImage, ConsultationResponse, Specialist
...
def home(request):
    ...
    #Specialists
    specialists = Specialist.objects.filter(status=True).all().order_by('order')
...
def requests_history(request):
    if request.user.is_authenticated:
        totalRequests = ConsultationResponse.objects.filter(user=request.user)
        
        #print(results)
        context = {
            'obj_list': totalRequests
        }
        return render(request, "consultation/requests_history.html", context)
    else:
        return HttpResponseRedirect(reverse('consultation:login'))
...
def requests_list(request):
    if request.user.is_authenticated:
        ...
        total=new_requests=answered=rejected = 0
        results = Consultation.objects.filter(treatment_type=user_type, status='1').order_by('-id')
        new_requests = Consultation.objects.filter(treatment_type=user_type, status='1').count()
        answered = ConsultationResponse.objects.filter(user=request.user, response_type='answered').count()
        rejected = ConsultationResponse.objects.filter(user=request.user, response_type='rejected').count()
        total = ConsultationResponse.objects.filter(user=request.user).count()
