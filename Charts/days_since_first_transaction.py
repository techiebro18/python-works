...
try:
    ...
    emails = df['email']
    last_transactions = df['days_since_first_transaction'].sort_values(ascending=True)
    plt.subplots_adjust(bottom=0.30)
    plt.xticks(rotation=60, fontsize=6)
    plt.bar(emails, last_transactions, label = 'Days')
    plt.xlabel("Users")
    plt.ylabel("Days since first transaction")
    plt.title("Top 50 Users with Number of days since first transaction")
    plt.legend()
    plt.show()
